...
def main(mode, args):
    ...
    classes = [1, 254, 323, 387, 972, 980]
    for class_label in classes:
        class_labels = [class_label]

        os.makedirs(os.path.join(args.save_dir, "save_dir", str(class_label)), exist_ok=True)

        # Create sampling noise:
        n = len(class_labels)
        for t in range(0, args.times):
            #z = torch.randn(n*n_sample, args.embed_dim, latent_size, latent_size, device=device)
            z = torch.randn(n_sample, args.embed_dim, latent_size, latent_size, device=device)
            ...
            for i in range(0, samples.shape[0]):
                save_image(samples[i].unsqueeze(0), os.path.join(args.save_dir, "save_dir", str(class_label), "%s_%s.jpg")%(str(count), str(i + t * n_sample)), nrow=1, normalize=True, value_range=(-1, 1))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--vq_config_path", type=str, default="vqgan_configs/v2l_256.yaml")
